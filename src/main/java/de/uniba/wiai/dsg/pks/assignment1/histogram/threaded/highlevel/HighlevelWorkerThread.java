...
import de.uniba.wiai.dsg.pks.assignment1.shared.OutputThread;
...
@ThreadSafe
public class HighlevelWorkerThread extends Thread {

    private final Path path;
    private final String fileExtension;
    @GuardedBy(value = "itself")
    private final LinkedBlockingQueue<Histogram> histograms;
    @GuardedBy(value = "itself")
    private final OutputThread out;

    public HighlevelWorkerThread(Path path, String fileExtension, LinkedBlockingQueue<Histogram> histograms) {
        this.path = path;
    ...
    public void run() {
        ...
        out.print("Directory: \t" + path.toString() + " finished!\n [distr: " + Arrays.toString(distribution) +
                ", lines=" + lines + ", files=" + files + ", processedFiles=" + processedFiles + ", directories=1]");
